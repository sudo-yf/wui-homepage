...
export async function createLetterPlayground({ scene, camera, container, arms, wake, reducedMotion }) {
  ...
  return { homeBounds, update, setPalette, nextWake() { return enabled && !reducedMotion.matches && Number.isFinite(nextAction) ? Math.max(nextAction, nextAssignment) : null; }, suspend() { release(); cancelJob(); }, dispose() { disposed = true; cancelJob(); } };
}
